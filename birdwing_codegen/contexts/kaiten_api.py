...
def add_param_metadata(api):
    for fidx, func in enumerate(api):
        ...
        for pidx, param in enumerate(func['parameters']):
            ...
            # 'default' key. the 'default' key is often set to null, indicating
            # the param is optional. this causes issues, as mustache evaluates
            # both the absense of keys and keys set to null as False.  A null
            # key vs an absent key has different semantics, so create a bool
            # for params that explicitly indicates whether its optional.
            api[fidx]['parameters'][pidx]['optional'] = 'default' in param
    return api
...
def generate_context(env, target, source):
    ...
    api = json.loads(subprocess.check_output(['python3', apigen_path, '-l 3']))
    api_dict['kaiten_api'] = add_param_metadata(api)
    return api_dict
